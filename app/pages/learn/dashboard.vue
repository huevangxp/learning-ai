    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
      ...
      >
        <div class="flex items-center justify-between mb-2">
          <div
            :class="['w-10 h-10 rounded-xl flex items-center justify-center border', stat.colorClass]"
          >
        ...
        <div class="absolute bottom-0 left-0 right-0 h-16 opacity-20 pointer-events-none group-hover:opacity-40 transition-opacity">
    ...
    <!-- ========== TOPIC MASTERY BREAKDOWN (REPLACED LISTS) ========== -->
    <div class="grid lg:grid-cols-3 gap-8">
      <div class="lg:col-span-2 bg-white rounded-[2.5rem] border border-slate-200 p-8 shadow-sm relative overflow-hidden">
        <div class="flex items-center justify-between mb-8">
          <div>
            <h2 class="text-xl font-extrabold text-slate-900 tracking-tight">Topic Mastery Breakdown</h2>
            <p class="text-sm text-slate-500 font-medium">Visual distribution of your learning skills</p>
          </div>
          <div class="flex items-center gap-2">
            <div class="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-slate-50 border border-slate-100">
              <div class="w-2 h-2 rounded-full bg-indigo-500"></div>
              <span class="text-[10px] font-bold text-slate-500 uppercase">Tenses</span>
            </div>
            <div class="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-slate-50 border border-slate-100">
              <div class="w-2 h-2 rounded-full bg-purple-500"></div>
              <span class="text-[10px] font-bold text-slate-500 uppercase">Grammar</span>
            </div>
          </div>
        </div>
        ...
        <div class="h-64 flex items-end justify-between gap-4 py-4 px-2">
          <div v-for="bar in masteryBars" :key="bar.label" class="flex-1 flex flex-col items-center group">
            <div class="relative w-full flex flex-col items-center">
              ...
              >
                <!-- Tooltip -->
                <div class="absolute -top-10 left-1/2 -translate-x-1/2 bg-slate-900 text-white text-[10px] font-bold py-1 px-2 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-20 shadow-xl">
                  {{ bar.percent }}% Complete
                </div>
              </div>
            </div>
            <span class="text-[10px] font-bold text-slate-400 mt-4 uppercase rotate-[-45deg] lg:rotate-0 tracking-tighter">{{ bar.label }}</span>
          </div>
        </div>
      </div>

      <!-- Quick Learning Card -->
      <div class="bg-gradient-to-br from-indigo-600 to-purple-700 rounded-[2.5rem] p-8 text-white relative overflow-hidden group">
        ...
        <div class="relative z-10 flex flex-col h-full">
          <div class="w-12 h-12 rounded-2xl bg-white/20 backdrop-blur border border-white/20 flex items-center justify-center mb-6">
            <Icon name="ph:lightning-fill" class="text-2xl" />
          </div>
          <h3 class="text-2xl font-black mb-2">Daily Recommendation</h3>
          <p class="text-indigo-100 text-sm font-medium mb-8 leading-relaxed">
            Your radar shows <strong>Reading</strong> and <strong>Writing</strong> need focus today.
          </p>
          
          <div class="space-y-3 mb-8">
            <div class="flex items-center gap-3 p-3 rounded-2xl bg-white/10 border border-white/10">
              <div class="w-7 h-7 rounded-lg bg-emerald-400 text-slate-900 flex items-center justify-center text-xs font-black">1</div>
              <p class="text-xs font-bold">5min Reading Quiz</p>
            </div>
            <div class="flex items-center gap-3 p-3 rounded-2xl bg-white/10 border border-white/10">
              <div class="w-7 h-7 rounded-lg bg-amber-400 text-slate-900 flex items-center justify-center text-xs font-black">2</div>
              <p class="text-xs font-bold">Active Voice Exercise</p>
            </div>
          </div>

          <button class="mt-auto w-full py-4 rounded-2xl bg-white text-indigo-600 font-extrabold hover:bg-slate-50 transition active:scale-95 shadow-xl">
    ...
    <div class="grid lg:grid-cols-3 gap-8">
      ...
      <div class="bg-white rounded-[2.5rem] border border-slate-200 p-8 shadow-sm">
        <h3 class="text-sm font-bold text-slate-400 uppercase tracking-widest mb-8">Activity Intensity</h3>
        <div class="relative h-48 w-full group">
          <!-- Activity Line Chart (Simplified) -->
        ...
        <div class="flex justify-between mt-4">
          <span v-for="day in weekData" :key="day.label" class="text-[10px] font-bold text-slate-400 uppercase">{{ day.label }}</span>
        </div>
        <div class="mt-8 pt-8 border-t border-slate-100 flex items-center justify-between">
          <div>
            <p class="text-xs font-bold text-slate-400 uppercase tracking-wider">Total Time</p>
      ...
      <!-- Center Column: Skill Radar Chart (Pentagon) -->
      <div class="space-y-6">
        <div class="bg-white rounded-[2.5rem] border border-slate-200 p-8 shadow-sm">
          <h3
      ...
      <div class="space-y-6">
        <div
          class="bg-white rounded-[2.5rem] border border-slate-200 p-8 shadow-sm text-center relative overflow-hidden h-full flex flex-col items-center justify-center"
        >
          <div class="absolute -top-12 -right-12 w-32 h-32 bg-indigo-50 rounded-full blur-3xl"></div>
          <h3
          ...
          <p class="text-xs font-medium text-slate-500 mt-10 max-w-[180px]">
            You're <strong>320 XP</strong> away from reaching <strong>B2 Upper Intermediate</strong>.
          </p>
        </div>
      </div>
    </div>
...
const dailyTasks = ref([
  {
    label: "Practice 5 mins Speaking",
    xp: "+20",
    icon: "ph:microphone",
    done: true,
  },
  {
    label: "Complete Vocabulary Set",
    xp: "+15",
    icon: "ph:book-open",
    done: true,
  },
  { label: "Read a short article", xp: "+25", icon: "ph:article", done: false },
  { label: "Grammar Quick Quiz", xp: "+30", icon: "ph:brain", done: false },
]);
...
const badges = ref([
  ...
  },
  {
    emoji: "🎯",
    title: "First Exam",
    desc: "Passed your first level test",
    bg: "bg-indigo-100",
  },
  {
    emoji: "💬",
    title: "AI Chatter",
    desc: "50 conversations with AI tutor",
    bg: "bg-purple-100",
  },
]);

const leaderboard = ref([
  { name: "Sarah K.", avatar: "SK", xp: "4,820" },
  { name: "Miguel R.", avatar: "MR", xp: "3,910" },
  { name: "John Doe", avatar: "JD", xp: "2,450" },
  { name: "Yuki T.", avatar: "YT", xp: "2,100" },
]);
